...
class S3:
    ...
    def upload_file(self, local_path, s3_path):
        ...
        try:
            ...
            # 上传时带上 Content-Type
            bucket.upload_file(local_path, s3_path, ExtraArgs={"ContentType": content_type})

            logger.info(f"Uploaded {local_path} → s3://{self.bucket_name}/{s3_path} (Content-Type: {content_type})")
            return s3_path
